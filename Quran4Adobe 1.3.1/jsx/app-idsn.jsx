...
function addTextFrame(content, fontName, fontSize) {
    ...
    // Convert the margin to the document's current measurement units
    var margin = convertUnits(1, MeasurementUnits.INCHES, horizontalUnits);
    
	// Check if the new frame is larger than the page width
    if (frameWidth > pageWidth || frameHeight > pageHeight) {
        // If so, use default margins
        ...
        // If not, center the frame
        textFrame.geometricBounds = [
            newCenterY, // Top
            newCenterX, // Left
            newCenterY + frameHeight, // Bottom
            newCenterX + frameWidth   // Right
		];
	}
}
...
function convertUnits(value, fromUnits, toUnits) {
    ...
    switch (fromUnits) {
		...
		// TEST TODO add more cases if needed for other unit conversions
		break;
...
function addVerseIDSN(verseKey, fontSize, parentheses, ayaKaws) {
